...
impl Raster {
	...
	pub fn add_point(&mut self, x: i16, y: i16, value: String) {
		self.grid.insert((x, y), value);
	}
	
	/// Checks if a point exists in the raster
	pub fn has_point(&self, x: i16, y: i16) -> bool {
		self.grid.contains_key(&(x, y))
	}
	
	/// Gets a value at a specific point if it exists
	pub fn get_point(&self, x: i16, y: i16) -> Option<&String> {
		self.grid.get(&(x, y))
	}
	...
	pub fn cast_ray(&self, start_x: f32, start_y: f32, angle: f32, max_distance: f32) -> Vec<(i16, i16)> {
		...
		while distance < max_distance {
			...
			let grid_x = x.round() as i16;
			let grid_y = y.round() as i16;
	...
	pub fn create_boundary(&mut self, width: u16, height: u16, border_char: &str) {
		...
		for x in 0..width {
			self.add_point(x as i16, 0, border_char.to_string());
			self.add_point(x as i16, (height - 1) as i16, border_char.to_string());
		}
		
		// Create left and right walls
		for y in 0..height {
			self.add_point(0, y as i16, border_char.to_string());
			self.add_point((width - 1) as i16, y as i16, border_char.to_string());
		}
	}
	...
	pub fn to_string_with_ray(&self, field: &Field, start_x: f32, start_y: f32, angle: f32, max_distance: f32) -> String {
		...
		let ray_set: std::collections::HashSet<(i16, i16)> = ray_points.into_iter().collect();
		...
		for y in min_y..=max_y {
			for x in min_x..=max_x {
				...
				if ray_set.contains(&(x, y)) {
					...
					if let Some(entity) = field.get_entity_by_position(x, y) {
				...
				} else if let Some(entity) = field.get_entity_by_position(x, y) {
	...
	pub fn to_2d5_view(&self, _field: &Field, start_x: f32, start_y: f32, angle: f32, fov: f32, width: usize, height: usize) -> String {
		// Define constants
		...
		// Center the main ray in the middle column
		let center_column = width / 2;

		// Cast rays across the field of view
		for column in 0..ray_count {
			// Calculate ray angle so that the center ray is at the given angle
			let ray_angle = angle + ((column as isize - center_column as isize) as f32) * angle_step;
