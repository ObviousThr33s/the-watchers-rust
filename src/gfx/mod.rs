...
pub fn render(terminal: &mut ratatui::DefaultTerminal, log:&Logger, field:&Field, viewport_text:&String, player_pos:(i16, i16), portal:&Portal, inventory:&[crate::game::item::Item], selection:ui::PanelSel) {
	//thank you ratatui for TUI-ing so I could TUI something cool with the robots.
	let _ = terminal.draw(|frame| ui::draw_(frame, viewport_text, field, log, player_pos, portal, inventory, selection) );
